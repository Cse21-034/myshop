...
export default function CartDrawer({ isOpen, onClose }: CartDrawerProps) {
  ...
  // Convert USD string price to BWP number
  const convertToBWP = (usdPrice: string | undefined): number => {
    if (!usdPrice) return 0;
    const numberPrice = parseFloat(usdPrice);
    return isNaN(numberPrice) ? 0 : numberPrice * USD_TO_BWP;
  };

  // Format BWP number to string with P prefix and 2 decimals
                  ...
                  <div key={item.id} className="flex items-center space-x-4 p-4 border border-gray-200 rounded-lg">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-secondary font-semibold">
                        {/* Convert and format price per unit * quantity */}
                        {formatBWP(convertToBWP(item.product?.price) * item.quantity)}
                      </p>
                    </div>
            ...
            <div className="border-t pt-4 mt-4">
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <span>Subtotal:</span>
                  <span>{formatBWP(subtotal * USD_TO_BWP)}</span>
                </div>
                <div className="flex justify-between">
                  <span>Shipping:</span>
                  <span>{shipping === 0 ? "Free" : formatBWP(shipping * USD_TO_BWP)}</span>
                </div>
                <div className="flex justify-between">
                  <span>Tax:</span>
                  <span>{formatBWP(tax * USD_TO_BWP)}</span>
                </div>
                <Separator />
                <div className="flex justify-between font-bold text-lg">
                  <span>Total:</span>
                  <span className="text-secondary">{formatBWP(total * USD_TO_BWP)}</span>
                </div>
              </div>
