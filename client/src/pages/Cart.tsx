...
export default function Cart() {
  ...
  const subtotal = cartItemsWithProducts.reduce((total, item) => {
    return total + (item.product ? convertToBWP(item.product.price) * item.quantity : 0);
  }, 0);

  const shipping = subtotal > 75 * USD_TO_BWP ? 0 : 9.99 * USD_TO_BWP;
                        ...
                        <div className="flex items-center space-x-4">
                          <div className="relative">
                            <img
                              src={
                                ...
                                "https://images.unsplash.com/photo-1441986300917-64674bd600d8?ixlib=rb-4.0.3&auto=format&fit=crop&w=150&h=150"
                              }
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-secondary font-bold text-lg">
                              {/* Convert and format single unit price */}
                              {item.product ? formatBWP(convertToBWP(item.product.price)) : "-"}
                            </p>
                          </div>
                          ...
                          <div className="text-right">
                            <p className="font-bold text-lg">
                              {/* Convert and format total item price */}
                              {item.product ? formatBWP(convertToBWP(item.product.price) * item.quantity) : "-"}
                            </p>
                            <button
                ...
                <CardContent>
                  <div className="space-y-4">
                    <div className="flex justify-between">
                      ...
                      <span>{formatBWP(subtotal)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Shipping:</span>
                      <span>{shipping === 0 ? "Free" : formatBWP(shipping)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Tax:</span>
                      <span>{formatBWP(tax)}</span>
                    </div>
                    <Separator />
                    <div className="flex justify-between text-lg font-bold">
                      <span>Total:</span>
                      <span className="text-secondary">{formatBWP(total)}</span>
                    </div>

                    {subtotal < 75 * USD_TO_BWP && (
                      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 mt-4">
                        <p className="text-sm text-yellow-800">
                          Add {formatBWP(75 * USD_TO_BWP - subtotal)} more to get free shipping!
                        </p>
                      </div>
                    )}
                  </div>
